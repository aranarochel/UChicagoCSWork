...
structure PrintParseTree : sig
  ...
  (* dump parse-tree to the standard output for debugging purposes; this will not show marks *)
  ...
  (* print a list of items enclosed in "[" ... "]" *)
    ...
    fun program (strm as S{info=Info{outS, ...}, ...}, p) = let
	  fun prog (strm, PT.MarkProgram m) = program (mark (strm, m))
	    | prog (strm, PT.Program(defs, e)) = (
		prNode (strm, "Program");
		nest strm (fn strm => (
  ...
  (* dump a a parse-tree term to the standard output for debugging purposes *)
