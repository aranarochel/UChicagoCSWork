  ...
  structure BN = BasisNames
    ...
    fun typToTy (typ) = (case typ
        of ParseTree.MarkTyp {span = sp, tree = t} => typToTy t
         ...
         | ParseTree.ConTyp (id, typs)    => let
             fun recur (typs, acc) = (case typs 
                of [] => Type.TupleTy(rev acc)
                 | t::ts => recur(ts,(typToTy(t)::acc))
                 (*end case*))
             in recur(typs, [])
             end
    ...
    and chkDefs (cxt, defs) = (case defs
        of [] => cxt   
         ...
         | (ParseTree.ValDef  bind)::rest 
            => let val (ast,ctx') = chkBind(cxt, bind)
               in chkDefs(ctx', rest)
               end
         (*end case*))
        ...
        of (ParseTree.MarkExp dm)
         ...
         | (ParseTree.IfExp (exp1, exp2, exp3)) (*6.1 j1*)
            =>  let val (expRep1,exp1Ty) = chkExp(ctx,exp1)
                ...
                in
                    ...
                    then (AST.IfExp(AST.Exp(expRep1,exp1Ty),
                                    AST.Exp(expRep2,exp2Ty),
                                    AST.Exp(expRep3,exp3Ty)),
                          exp3Ty)
                    else (AST.IfExp(AST.Exp(expRep1,exp1Ty),
                                    AST.Exp(expRep2,exp2Ty),
                                    AST.Exp(expRep3,exp3Ty)),
                          Type.ErrorTy) (*raise Fail "Type error on chkExp.IfExp"*)
                end
         | (ParseTree.BinExp (exp1, id, exp2)) => (case Atom.toString(id)
                of "::" =>  let val (expRep1,exp1Ty) = chkExp(ctx, exp1)
                                val (expRep2,exp2Ty) = chkExp(ctx, exp2)
                            in (case exp2Ty
                                    of Type.TupleTy tys 
                                        => if (cmpTyToTys(exp1Ty, tys))
                                           then (AST.TupleExp(AST.Exp(expRep1,exp1Ty)::
                                                              AST.Exp(expRep2,exp2Ty):: 
                                                              nil),
                                                exp2Ty)
                                           else (AST.TupleExp(AST.Exp(expRep1,exp1Ty)::
                                                              AST.Exp(expRep2,exp2Ty)::
                                                              nil),
                 ...
                 | _    =>  let val (expRep1,exp1Ty) = chkExp(ctx, exp1)
                            ...
                            in 
                                ...
                                then (AST.AppExp(AST.Exp(AST.VarExp(Var.new(id,exp1Ty)),Type.FunTy(Type.TupleTy(exp1Ty::exp1Ty::nil),exp2Ty)),
                                                 AST.Exp(AST.TupleExp(
                                                        AST.Exp(expRep1,exp1Ty)::
                                                        AST.Exp(expRep2,exp2Ty)::
                                                        nil),exp2Ty)
                                                 ),
                                      exp1Ty)
         ...
         | (ParseTree.AppExp (exp1, exp2)) (*6.7 j3?*)
            =>  let val (expRep1,exp1Ty) = chkExp(ctx, exp1)
                    val (expRep2,exp2Ty) = chkExp(ctx, exp2)
                in 
                    if TypeUtil.same(exp1Ty, exp2Ty)
                            then (AST.AppExp(AST.Exp(expRep1,exp1Ty),
                                             AST.Exp(expRep2,exp2Ty)),
                                  exp2Ty)
                            else (AST.AppExp(AST.Exp(expRep1,exp1Ty),
                                             AST.Exp(expRep2,exp2Ty)),
                                  Type.ErrorTy) 
                    (*(case exp1Ty
            ...
            =>  (case (chkExp(ctx,exp))
                     of (expRep, Type.FunTy(Type.PolyTy(tyvars, ty),ty2)) 
                        =>  let fun mapChkTyp (ctx, typs) = 
                                    let fun map' (ctx,typs, accTys) = (case typs 
                                        of [] => rev accTys
                                         | (t::rest) => let val  expTy' = chkTyp(ctx,t)
                                                        in map'(ctx,rest,expTy'::accTys)
                                                        end
                                        (*end case*))
                                    in
                                        map'(ctx,typs,[])
                                    end
                                val tys = mapChkTyp(ctx, typs)
                                val (expRep, expTy) = chkExp(ctx,exp)
                            in (AST.TyAppExp(AST.Exp(expRep, expTy), tys),
                                TypeUtil.apply(tyvars, ty2, tys)) (*double check this*)
                            end 
                      | _ => (AST.TyAppExp(AST.Exp(chkExp(ctx,exp)),nil),
         ...
         | (ParseTree.VarExp (id))
            =>  (case AtomMap.find(getVE(ctx), id)
                    of SOME var => (AST.VarExp(var),
                                    Var.typeOf(var))
                     | NONE => (AST.VarExp(Var.new(Atom.atom("id not found in VE"),Type.ErrorTy)),
         ...
         | (ParseTree.ConExp (id)) 
            =>  (case AtomMap.find(getDCE(ctx), id)
                    of SOME dc => (AST.ConExp(dc), 
                                   DataCon.typeOf(dc))
                     | NONE => (AST.VarExp(Var.new(Atom.atom("id not found in DCE"),Type.ErrorTy)),
         ...
         | (ParseTree.TupleExp (exps)) 
            =>  let fun mapChkTuple (ctx, exps, accTys, accReps) = (case exps
                        of [] => (rev accReps, rev accTys)
                         | e::rest => let val (expRep, expTy) = chkExp(ctx,e)
                                      in mapChkTuple(ctx, rest, expTy::accTys, expRep::accReps)
                                      end
                        (*end case*))
                    val (expReps, expTys) = mapChkTuple(ctx, exps, [], [])
                    val repsTysList = ListPair.zip(expReps,expTys)
                    val expifyRTL = map (fn (a,b) => AST.Exp(a,b)) repsTysList
                in (AST.TupleExp(expifyRTL),
                    Type.TupleTy(expTys))
                end (*AST.Exp(expReps,expTys)*)
         | (ParseTree.CaseExp (exp, case_matches)) 
            =>  let val (expRep,expTy) = chkExp(ctx, exp)
                ...
                in (AST.CaseExp(AST.Exp(expRep,expTy), patExp), 
                    expTy)
                end
            ...
            fun loop (cx, t, cs) = (case cs                
                of [] => [] 
                 ...
                 | (ParseTree.CaseMatch(pat,scope))::rest 
                    => let val (p, cxt')  = chkPat (cx, t, pat)
                           val cxt'' = envMerge (cx, cxt')
                           val (bn,exp,newTy) = chkScope (cxt'', scope)
                           val ret = (AST.Rule(p,exp), newTy)
                        in
                           (ret::loop(cx, t, rest))
                        end)

            val ret = loop (cxt, ty, cases)
            val (tmp,ty)  = foldr (fn((x1,x2),(y1,y2)) => 
                    if TypeUtil.same(x2,y2) then (x1,x2) else raise Fail "error: scope types don't match")
                      (hd ret) (tl ret)
            val rs  = (map (fn (a,b) => a) ret)
            (*(foldr (fn((x1,x2),(y1,y2)) => (x1::y1,y2)) (hd ret) (tl ret))*)
    ...
    and chkBind (ctx, bind) = (case bind 
        of (ParseTree.MarkBind dm)                    
         ...
         | (ParseTree.FunBind (id, params, typ, exp)) (*fun id (params) : typ = exp *)
            => let val (ps,ctx', typ', retTyp) = chkFnSig(ctx,id,params,typ)
                   val (retRep,retTyp') = chkExp(ctx', exp)
                   val ret = AST.FunBind(Var.new(id,typ'), ps, AST.Exp(retRep,retTyp'))
                in if TypeUtil.same(retTyp, retTyp')
                   then (ret, updateVE(ctx, id, typ))
                   else raise Fail "function binding types do not match"
                end
            ...
            => (case maybeTyp
                of SOME(typ) 
                    => let  val typTy = chkTyp(ctx,typ)
                            val (expRep,expTy) = chkExp(ctx, exp)
                            val (AST.VarPat v,ctx')  = chkSpat(ctx, typTy, pat)
                            val ret = AST.ValBind(v, AST.Exp(expRep,expTy))
                        in if (TypeUtil.same(typTy, expTy))
                           then (ret,envMerge (ctx, ctx'))
                           else raise Fail "Error: pattern typ and exp typ do not match in ValBind"
                        end 
                 | NONE      
                    => let val (expRep,expTy) = chkExp(ctx, exp)
                           val (AST.VarPat v,ctx')  = chkSpat(ctx, expTy, pat)
                           val ret = AST.ValBind(v, AST.Exp(expRep,expTy))
                       in (ret,envMerge (ctx, ctx'))
                       end
                 (*end case*))
         | (ParseTree.ExpBind exp)
            => let val (expResultRep,expResultTy) = chkExp(ctx,exp) 
               in if TypeUtil.same(expResultTy, Basis.unitTy) 
                  then (AST.DataBind(Basis.unitTyc), ctx)
                  else raise Fail "todo: prevent error propagation when ExpBind !: unit"
               end 
        (*end case*))

    and chkFnSig (ctx,id,params,typ) = (case  params
        of [] => let val typTy = chkTyp(ctx, typ)
                 in ([AST.ValParam(Var.new(id,typTy))],ctx, Type.ErrorTy, Type.ErrorTy)
                 end
         ...
         | (ParseTree.TyParam(ids))::rest => let val ctx' = updateTVE(ctx, ids)
                                                 ...
                                                 val (ast,ctx'', funTy, retTy) = chkFnSig(ctx',id,rest,typ)
                                                 val tvs = map (TyVar.new) ids
                                             in ([AST.TyParam tvs],ctx'', funTy, retTy)
                                             end 
         | (ParseTree.ValParam(id',typ))::rest => let val typTy = chkTyp(ctx, typ)
                                                      val ctx' = updateTVE(ctx,id'::nil) (*should this by updateTCE somehow?*)
                                                      val (ast,ctx'', funTy, retTy) = chkFnSig(ctx',id,rest,typ) (*should recur through all params, right?*)
                                                  in ([AST.ValParam(Var.new(id',typTy))],ctx'', funTy, retTy)
                                                  end
        (*end case*))
         ...
         | ParseTree.Scope(binds, ex) => 
                let 
                    fun loop (ctx, bns) = (case bns
                        of b::[]   => let val (bin,cxt') = chkBind(cxt,b)
                                      in
                                          (bin,cxt')
                                      end
                         | b::rest => let val (bin,cxt') = chkBind(cxt,b)
                                      in loop(cxt', rest)
                                      end
                         | _      => raise Fail "hey")

                    val (bn,cxt2) = loop(cxt,binds)
                    val (exp_rep,ty) = chkExp(cxt2,ex)
                    val ret = AST.Exp(exp_rep,ty)
                in 
                    (bn,ret,ty)  
                end
        (* end case *))
