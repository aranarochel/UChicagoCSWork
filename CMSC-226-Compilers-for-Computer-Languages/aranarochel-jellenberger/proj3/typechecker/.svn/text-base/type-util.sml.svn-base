...
(*
    fun apply ([], ty, []) = copy ty
      | apply (params, ty, args) = let 
            fun findParams (ty,params) = (case ty 
              of Type.PolyTy(tyvar,ty) => findParams(ty,(tyvar::params))
               | Type.VarTy(tyvar) => rev (tyvar::params)
               | Type.ConTy(tycon,tys) => (case tys
                  of [] => rev params
                   | t::ys => findParams(ys, (t::params))
                   )
               | Type.FunTy(ty1,ty2) => findParams(ty2, params)
               | Type.TupleTy(tys) => (case tys
                  of [] => rev params
                   | t::ys => findParams(ys, (t::params))
                   )
               | Type.ErrorTy() => params
            val tyParams = findParams(ty, [])
        in
          replace
        end 
*)
