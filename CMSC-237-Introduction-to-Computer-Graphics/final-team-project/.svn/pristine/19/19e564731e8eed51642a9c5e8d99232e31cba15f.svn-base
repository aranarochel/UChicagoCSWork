...
// a transformation matrix that assumes that the camera is at the origin.
cs237::mat4x4f Camera::viewTransform (cs237::vec3d pos) const
{   
    return cs237::lookAt (
    cs237::vec3f(pos.x,pos.y,pos.z),
    this->_dir + cs237::toFloat(pos),
    this->_up);
}

// a transformation matrix that assumes that the camera is at the origin.
cs237::mat4x4f Camera::viewTransform (cs237::vec3d pos, cs237::vec3f dir) const
{
    return cs237::lookAt (
    cs237::vec3f(pos.x,pos.y,pos.z),
    dir,// + cs237::toFloat(pos),
    this->_up);
}
...
void Camera::look (cs237::vec4f const &dir)
{
    hasMoved=true;
    this->_dir = normalize(cs237::vec3f(dir));
}
