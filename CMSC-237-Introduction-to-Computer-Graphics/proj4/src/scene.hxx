...
class Scene {
  ...
  //! return the number of spot lights in the scene
    int NumLights () const {return this->_lights.size(); }

  //! iterator for looping over the spotlights in the scene
    std::vector<SceneLight>::const_iterator beginLights () const { return this->_lights.begin(); }

  //! terminator for looping over the objects in the scene
    std::vector<SceneLight>::const_iterator endLights () const { return this->_lights.end(); }
  ...
  private:
    ...
    std::vector<SceneLight> _lights;		//!< the lights in the scene
