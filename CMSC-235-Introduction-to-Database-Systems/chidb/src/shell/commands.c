...
struct handler_entry handlers[] =
{
    ...
    HANDLER_ENTRY (schema,    ".schema            show da schema list"),
    HANDLER_ENTRY (quit,      ".quit              quit da program yo"),
...
int chidb_shell_handle_cmd_quit(chidb_shell_ctx_t *ctx, struct handler_entry *e, const char **tokens, int ntokens)
{
    ...
    // return CHIDB_OK; // this will never be returned but handlers need to be of type int i think.
}
