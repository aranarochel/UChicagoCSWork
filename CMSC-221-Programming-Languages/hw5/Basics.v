...
Proof.
  ...
  simpl. intros H. rewrite -> H. reflexivity. (* Second subgoal *)
  simpl. intros H. rewrite -> H. reflexivity. (* Third subgoal *)
  Qed.
...
Inductive bin : Type :=
  | Zero : bin         (* Zero *)
  | T : bin -> bin  (* Twice a binary bnum *)
  | A : bin -> bin. (* One more than twice a binary num *)

(* (b) *)
Fixpoint incr_bin (n : bin) : bin :=
  match n with
  | Zero => A Zero
  | T n' => A n'
  | A n' => T (incr_bin n')
...
(* (c) *)
Example test_incr_bin: incr_bin (A Zero) = T (A Zero).
Proof. reflexivity. Qed.

Example test_bin_2_un_1: bin_2_un (A Zero) = 1.
Proof. reflexivity. Qed.

Example test_bin_2_un_2: bin_2_un (T (T (A Zero))) = 4.
Proof. reflexivity. Qed.
