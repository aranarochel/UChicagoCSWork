...
They are very similar in that [destruct] and [induction] both allow for an analysis on different
cases for some variable. But [induction] produces an induction hypothesis for the variable's
subterms.
...
Theorem helper_theorem : forall n m : nat,
  m + m * n = m * S n.
Proof.
  intros n m.
  induction m as [ | n'].
