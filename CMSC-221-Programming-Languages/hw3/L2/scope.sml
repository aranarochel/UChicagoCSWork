    ...
    fun lp (env, t) = 
      (case t
         of AST.Int i => ScopedAST.Int i
          | AST.True => ScopedAST.True
          | AST.False => ScopedAST.False
          ...
          | AST.LNeg (exp) => ScopedAST.LNeg(lp(env,exp))
          | AST.IsZero (exp) => ScopedAST.IsZero(lp(env,exp))
          | AST.LessThan (e1,e2) => ScopedAST.LessThan(lp(env,e1),lp(env,e2))
          | AST.If (e1,e2,e3) => ScopedAST.If(lp(env,e1),lp(env,e2),lp(env,e3)))
    in
      lp ([],t)
    end
end
